...
predictPositiveMarker <- function(sce_object, plot_actual_cutoff = FALSE, plot_predicted_cutoff = FALSE, thresholds = NULL, tumour_marker,
                               baseline_markers, viewThreshold = FALSE) {
    ...
    if (!is.null(thresholds)) {
      ...
      print(unname(cutoff_for_tumour))
      print(selected_valley_xcord)
    ...
    # The threshold used
    markerThresh <- data.frame(marker_vector, threshold_vector)
    if (viewThreshold == TRUE){
      print(markerThresh)
    }
